...
def neigh_freq_and_expression(sample : pd.DataFrame, tpm : pd.DataFrame, prob_matrix : pd.DataFrame, granges : list, flank : int, org : str, read_size : int) -> pd.DataFrame:
  ...
  for i, grange in enumerate(granges):
    ...
    print(sample.loc[cur_flank_both, 'Position'])
...
def junction_only(sample : pd.DataFrame, remove_trna : bool) -> pd.DataFrame:
  """
  """
  tdict = {k: 'first' for k in ['org', 'coexpression', 'left_tpm', 'right_tpm', 'left_rpkm','right_rpkm', 'right_gene', 'left_gene', 'neigh_freq', 'strand_switch', 'Chromosome', 'Feature', 'junction_size','sum_around_junction']}
...
def main(args : argparse.ArgumentParser) -> None:
  ...
  junctions = junction_only(sample, args.no_trna)
